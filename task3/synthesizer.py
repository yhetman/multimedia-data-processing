...
def get_piano_notes():
    ...
    note_freqs = dict(zip(keys, [2 ** ((n + 1 - 49) / 12) \
        * base_freq for n in range(len(keys))]))
    note_freqs[''] = 0.0
    return note_freqs
...
def apply_overtones(frequency, duration, factor,
    sample_rate=44100, amplitude=4096):
    assert abs( 1 - sum(factor)) < 1e-8
    frequencies = np.minimum(np.array([ frequency * (x + 1)
     for x in range(len(factor))]), sample_rate // 2)
    amplitudes = np.array([amplitude * x for x in factor])
    fundamental = get_sine_wave(frequencies[0], duration, 
        sample_rate, amplitudes[0])
    for i in range(1, len(factor)):
        fundamental += get_sine_wave(frequencies[i], duration,
        sample_rate, amplitudes[i])
    return fundamental



def get_adsr_weights(frequency, duration, length, decay,
    sustain_level=0.1, sample_rate=44100):
    ...
    A = 1 / np.array([(1 - decay[0]) ** n for n in range(len_A)])   # Attack
    A = A / np.nanmax(A)

    D = np.array([(1 - decay[1]) ** n for n in range(len_D)]) # Decay
    D = D * (1 - sustain_level) + sustain_level

    S = np.array([(1 - decay[2]) ** n for n in range(len_S)]) # Sustain
    S = S * sustain_level

    R = np.array([(1 - decay[3]) ** n for n in range(len_R)]) # Release
    R = R * S[-1]
    ...
    if tail > 0:
        weights = np.concatenate((weights, \
            weights[-1] - weights[-1] / tail * np.arange(tail)))
    return weights
...
def apply_pedal(note_values, bar_value):
    ...
    while True:
        ...
        if end == 0:
            ...
            new_values += [bar_value - np.sum(this_bar[:i])
                for i in range(len(this_bar))]
        start += end + 1
...
def get_song_data(music_notes, note_values, \
    bar_value, sample_rate=44100, amplitude=4096):
    ...
    new_vals = apply_pedal(note_values, bar_value)
    duration = int(sum(note_values) * sample_rate)

    end_idx = np.cumsum(np.array(note_values) * \
        sample_rate).astype(int)
    start_idx = np.concatenate(([0], end_idx[:-1]))
    end_idx = np.array([start_idx[i] + \
        new_vals[i]*sample_rate for i in range(len(new_vals))]).astype(int)
    song = np.zeros((duration,))
    
    for i in range(len(music_notes)):
        this_note = apply_overtones(frequencies[i],\
            new_vals[i],factor)
        weights = get_adsr_weights(frequencies[i],\
            new_vals[i], length, decay, sustain_level)
